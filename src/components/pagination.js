...
export const initPagination = ({pages, fromRow, toRow, totalRows}, createPage) => {
    ...
    return (data, state, action) => {
        ...
        const visiblePages = getPages(page, pageCount, 5); // Массив номеров страниц для отображения

        pages.replaceChildren(...visiblePages.map(pageNumber => {
            const el = pageTemplate.cloneNode(true); // Клонируем наш заготовленный чертёж кнопки
            return createPage(el, pageNumber, pageNumber === page); // Заполняем данными через колбэк из main.js
        }));
